...
export default function RestaurantSlug() {
  ...
  let { query,asPath,pathname } = useRouter()
  let router = Test()
  console.log(router);

  let [slug1, slug2] = query?.restaurant

  const renderPage = () => {
    if (!slug2 && asPath ==="/restaurants") {
  ...
  return (
    <Fragment>
      <Head>
        ...
        <title> {t("restaurants")} {slug1?.toUpperCase()} | Foody Delivery</title>
      </Head>
      <Layout>
        {/* {renderPage()} */}
        {<RestaurantContainer />}
